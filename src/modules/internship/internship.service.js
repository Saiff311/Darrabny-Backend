...
export const addInternship = asyncHandler(async (req, res, next) => {
  ...
  if (
    ...
    return next(
      new Error("You are not authorized to add internship", { cause: 403 }),
    );
  }
...
export const getInternshipApp = asyncHandler(async (req, res, next) => {
  ...
  if (
    ...
    return next(
      new Error("You are not authorized to preform this action", {
        cause: 403,
      }),
    );
  }
...
export const ApplyToInternship = asyncHandler(async (req, res, next) => {
  ...
  const existApplication = await applicationModel.findOne({
    internshipId,
    userId: req.user._id,
  });
  if (existApplication) {
    return next(
      new Error("You have already applied for this internship", { cause: 403 }),
    );
  }
  ...
  io.to(internship.addedBy).emit("new-application", {
