...
export const signUp = asyncHandler(async(req, res, next)=>{
    const {firstName, lastName, email, password, gender, DOB, mobileNumber, role} = req.body
    ...
    //create user
    const user = await userModel.create({firstName, lastName, email, password, gender, DOB, mobileNumber, role})
    ...
    if (collegeId) {
        ...
        if (!college) { 
            return next(new Error("College ID doesn't exist in the system", { cause: 400 })); 
        }
    }
    //create student
    await studentModel.create({
        ...
        // check if college exists
        const college = await collegeModel.findById(collegeId);
        if (!college) return next(new Error("College ID doesn't exist in the system", { cause: 400 }));
        //create academic supervisor
        await academic_supervisorModel.create({ userId: user._id, collegeId, department, title });
    }
    else if (role === roles.company_supervisor) {
        const { companyId, position } = req.body; 
        // check if company exists
        const company = await companyModel.findById(companyId);
        if (!company) return next(new Error("Company ID doesn't exist in the system", { cause: 400 }));
        //create company supervisor
        await company_supervisorModel.create({ userId: user._id, companyId, position });
    }
...
export const confirmEmail = asyncHandler(async(req, res, next)=>{
    ...
    if (!user){
        return next(new Error("Email not exits or already confirmed",{cause:400}))
    }
...
export const login = asyncHandler(async (req, res, next)=>{
    ...
    //check email
    const user = await userModel.findOne({email , isConfirmed: true, provider: providers.system, deletedAt: null})
    if(!user){
        return next(new Error("This email isn't existed or not confirmed yet",{cause: 400}))
    }
    //check password 
    ...
    //Creating the token
    const accessToken = jwt.sign({email, id: user._id} , 
        user.role == roles.admin? process.env.ADMIN_SECRET_KEY: process.env.USER_SECRET_KEY , //*
        {expiresIn: "1h"})
    const refreshToken = jwt.sign({email, id: user._id} , 
        user.role == roles.admin? process.env.ADMIN_SECRET_KEY: process.env.USER_SECRET_KEY , //*
        {expiresIn: "7d"})
    return res.status(201).json({message: "done" , token: {accessToken, refreshToken}})
})
...
export const loginWithGmail = asyncHandler( async (req, res, next)=>{
    ...
    const {email, email_verified, picture, given_name, family_name} = await verify()   
    //signup
...
export const refreshToken = asyncHandler( async (req, res, next)=>{
    ...
    const user = await userModel.findOne({_id: payload.id, deletedAt: null}).select('-password').lean()
    if(!user){
...
export const resetPassword = asyncHandler(async (req, res, next) => {
    ...
    const now = new Date();
    const validOtp = user.otp.find(otp => otp.type === otpType.forgetPassword && otp.expiresIn > now);
    if (!validOtp) {
